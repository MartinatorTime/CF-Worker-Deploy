export async function checkServerStatusAndSendMessage(onlyNotifyIfDownOrMaintenance, chatId, replyToMessageId) {
  ...
  if (isServerInMaintenance && previousServerStatus !== 'Maintenance') {
    ...
    await sendStatusUpdateMessage(chatId, replyToMessageId, isServerDegraded, isServerInMaintenance);
...
export async function sendStatusUpdateMessage(chatId, replyToMessageId, isServerDegraded, isServerInMaintenance) {
...
export async function sendCurrentStatusMessage(chatId) {
  // Get the current server status from the KV store
  const currentServerStatus = await DATABASE.get('serverStatus');
  const messageText = `Current server status: ${currentServerStatus}`;
