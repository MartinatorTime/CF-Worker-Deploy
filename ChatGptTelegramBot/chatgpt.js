...
var ENV = {
  ...
  // 全局默认初始化消息
  SYSTEM_INIT_MESSAGE: "You are a helpful assistant, you know all, you can do all, you can write all",
  // 全局默认初始化消息角色
...
async function requestCompletionsFromOpenAI(message, history, context, onStream) {
  ...
  let resp = await fetch(`${ENV.API_DOMAIN}/v1/chat/completions`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
async function requestImageFromOpenAI(prompt, context) {
  ...
  const resp = await fetch(`${ENV.API_DOMAIN}/v1/images/generations`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "Authorization": `Bearer ${key}`
